...
public class BankSystem extends JFrame implements ActionListener, ItemListener {
	...
	public BankSystem () {
		...
		end = new JMenuItem ("Quit BankSystemApp ?", new ImageIcon ("Images/export.gif"));
		...
		about = new JMenuItem ("About BankSystemApp", new ImageIcon ("Images/Save.gif"));
		...
		report = new JMenuItem ("Print BankSystemApp Report", new ImageIcon ("Images/New.gif"));
		...
		btnKey.setToolTipText ("Shortcut Keys of BankSystemApp");
		...
		author = new JLabel (" " + "BankSystemApp.", Label.LEFT);
	...
	public void actionPerformed (ActionEvent ae) {
		...
		if (obj == addNew || obj == open || obj == btnNew) {
			...
			boolean b = openChildWindow ("BankSystemApp Help");
			if (b == false) {
				BankHelp hlpBank = new BankHelp ("BankSystemApp Help", "Help/Bank.htm");
				desktop.add (hlpBank);
				hlpBank.show ();
			}
			...
			boolean b = openChildWindow ("BankSystemApp Keys");
			if (b == false) {
				BankHelp hlpKey = new BankHelp ("BankSystemApp Keys", "Help/Keys.htm");
				desktop.add (hlpKey);
				hlpKey.show ();
			}
			...
			String msg = "BankSystemApp.\n\n" + "Created & Designed By:\n" + 
				"M.S. \n\n" + "E-mail me:\n SarsourM@gmail.com";
			JOptionPane.showMessageDialog (this, msg, "About BankSystemApp", JOptionPane.PLAIN_MESSAGE);
		...
		try {
			//Show a Confirmation Dialog.
		    	int reply = JOptionPane.showConfirmDialog (this,
					"Woud You Like to Exit\nFrom BankSystemApp?",
					"BankSystemApp - Exit", JOptionPane.YES_NO_OPTION, JOptionPane.PLAIN_MESSAGE);
			//Check the User Selection.
			if (reply == JOptionPane.YES_OPTION) {
				setVisible (false);	//Hide the Frame.
				dispose();            	//Free the System Resources.
				System.out.println ("Thanks for Using BankSystemApp\nAuthor - M.S.");
	...
	void getAccountNo () {
		...
		if (b == false) { }
			...
			try {
				...
				"(Tip: Account No. Contains only Digits)", "BankSystemApp - PrintRecord", JOptionPane.PLAIN_MESSAGE);
				if (printing == null) { }
				if (printing.equals ("")) {
					JOptionPane.showMessageDialog (this, "Provide Account No. to Print.",
						 "BankSystemApp - EmptyField", JOptionPane.PLAIN_MESSAGE);
					getAccountNo ();
				}
				else {
					findRec (printing);
		...
		try {
			...
			if (total == 0) {
				JOptionPane.showMessageDialog (null, "No Accounts Found.\nCreate an Account First.",
					 "BankSystemApp - Error", JOptionPane.PLAIN_MESSAGE);
				b = false;
			}
			else {
				b = true;
	...
	void findRec (String rec) {
		...
		if (found == false) {
			JOptionPane.showMessageDialog (this, "Account No. " + rec + " doesn't Exist.",
					 "BankSystemApp - WrongNo", JOptionPane.PLAIN_MESSAGE);
			getAccountNo ();
		}
	...
	String makeRecordPrint (int rec) {
		...
		String data6 = "          Copyright � 2023 M.S..\n";	//Page Footer.
		String sep0 = " -----------------------------------------------------------\n";
		String sep1 = " -----------------------------------------------------------\n";
		String sep2 = " -----------------------------------------------------------\n";
		String sep3 = " -----------------------------------------------------------\n";
